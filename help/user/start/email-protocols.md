...
>[!NOTE]
>
>이메일 전달성 서비스 및 컨설팅은 Adobe에서 제공하는 별도의 유료 서비스입니다. Journey Optimizer B2B edition 인스턴스에 대한 게재 능력 팀의 지원이 필요하거나 필요한 경우 해당 인스턴스에 대한 이메일 게재 능력 서비스 패키지(Essentials, Enhanced 또는 Plus) 중 하나를 구매해야 합니다. 이 구성은 기존 Marketo Engage 인스턴스의 모든 게재 기능 패키지와 독립적입니다. 전달성 서비스는 조직이 아닌 인스턴스별로 첨부됩니다. 두 인스턴스에서 게재 기능을 지원하려면 두 개의 별도 게재 기능 서비스 패키지가 필요합니다. Journey Optimizer B2B edition에 새 IP가 프로비저닝될 때마다 IP 준비 및 지속적인 게재 지원을 위해 새 게재 기능 서비스 패키지가 필요합니다.
...
1. SPF를 설정하려면 DNS 항목에 다음 줄을 추가합니다.
   ...
   DNS 항목에 SPF 레코드가 있는 경우 다음을 추가합니다.
...
또한 `FROM:` 도메인에 대한 DMARC 특정 DNS TXT 레코드를 구성합니다. 보고서 모니터링을 위해 조직 내에서 DMARC 보고서가 이동되는 위치를 지정하는 이메일 주소를 정의합니다(선택 사항).
