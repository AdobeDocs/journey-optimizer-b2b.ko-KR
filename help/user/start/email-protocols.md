...
전용 IP를 통해 Marketo Engage을 통해 메일을 보내고 브랜드 반환 경로를 구현하지 않은 경우(또는 반환 경로가 있는지 확실하지 않은 경우) [Adobe 지원](https://experienceleague.adobe.com/home?lang=en&amp;support-tab=home#support){target="_blank"}에서 티켓을 엽니다.
