---
title: 이메일 추적 및 게재 설정
description: 이메일 게재 프로토콜 구성 - Journey Optimizer B2B Edition에서 최적의 추적 및 게재를 위해 DNS, SPF, DKIM, DMARC 및 IP 허용 목록을 설정하십시오.
...
Adobe Journey Optimizer B2B edition은 연결된 Marketo Engage 인스턴스의 이메일 채널 기능 및 이벤트 추적을 활용합니다. 제한적인 방화벽 또는 프록시 서버 설정을 사용하는 조직에서 이메일 전달이 예상대로 작동하도록 하려면 시스템 관리자가 특정 도메인 및 IP 주소 범위를 허용 목록에 추가해야 합니다.
...
다음 단계를 완료하여 추적 및 이메일 전송을 확인합니다.

1. [랜딩 페이지 및 이메일에 대한 DNS 레코드 만들기](#create-dns-records-for-landing-pages-and-email)
...
>[!NOTE]
>
>이메일 전달성 서비스 및 컨설팅은 Adobe에서 제공하는 별도의 유료 서비스입니다. Journey Optimizer B2B edition 인스턴스에 대한 게재 능력 팀의 지원이 필요하거나 필요한 경우 해당 인스턴스에 대한 이메일 게재 능력 서비스 패키지(Essentials, Enhanced 또는 Plus) 중 하나를 구매해야 합니다. 이는 기존 Marketo Engage 인스턴스의 모든 게재 가능 패키지와 독립적입니다. 전달성 서비스는 조직이 아닌 인스턴스별로 첨부됩니다. 두 인스턴스에서 게재 기능을 지원하려면 두 개의 별도 게재 기능 서비스 패키지가 필요합니다. Journey Optimizer B2B edition에 새 IP가 프로비저닝될 때마다 IP 준비 및 지속적인 게재 지원을 위해 새 게재 기능 서비스 패키지가 필요합니다.
...
관리자는 마케팅 팀과 협력하여 두 개의 CNAME 레코드를 계획하고 구현해야 합니다. 첫 번째는 랜딩 페이지 URL용으로서, 랜딩 페이지는 Adobe Marketo Engage(실제 호스트)가 아니라 도메인을 반영하는 URL에 표시됩니다. 두 번째는 Marketo Engage을 통해 전송된 이메일에 포함된 추적 링크에 대한 것입니다.

### 랜딩 페이지에 대한 CNAME 추가

`[YourLandingPageCNAME]`이(가) 랜딩 페이지에 할당된 고유한 계정 문자열을 가리키도록 랜딩 페이지 CNAME를 DNS 레코드에 추가합니다. 도메인 등록자의 사이트에 로그인하고 랜딩 페이지 CNAME 및 계정 문자열을 입력합니다. 이 항목에는 일반적으로 다음 세 가지 필드가 포함됩니다.

* 별칭: `[YourLandingPageCNAME]` 입력
* 유형: CNAME
* 가리킨 항목: `[MunchkinID].mktoweb.com` 입력
...
>[!NOTE]
>
>`[MktoTrackingLink]` 값은 기본 [브랜딩 도메인](../admin/configure-channels-emails.md#branding-domains)이어야 합니다.

### SSL 인증서 프로비전

SSL 인증서 프로비전 프로세스를 시작하려면 [Adobe 지원 센터](https://experienceleague.adobe.com/home?lang=ko&support-tab=home#support){target="_blank"}에 문의하십시오.
...
1. DKIM의 경우 각 도메인에 대한 DNS 리소스 레코드를 만듭니다.
   ...
   Marketo Engage 문서의 [지침](https://experienceleague.adobe.com/ko/docs/marketo/using/product-docs/email-marketing/deliverability/set-up-a-custom-dkim-signature){target="_blank"}에 따라 각 DDIM 도메인에 대해 `HostRecord` 및 `TXTValue`를 복사합니다. Journey Optimizer B2B Edition에서 도메인을 확인할 수 있습니다([SPF/DKIM](../admin/configure-channels-emails.md#spfdkim) 참조).
...
* 유효한 SPF
* FROM의 유효한 DKIM 레코드: 도메인([!DNL Marketo Engage] 및 [!UICONTROL Journey Optimizer B2B edition]에 권장)

또한 `FROM:` 도메인에 대한 DMARC 특정 DNS TXT 레코드를 구성합니다. 원할 경우, 보고서 모니터링을 위해 조직 내에서 DMARC 보고서가 어디로 전송되어야 하는지 지정하는 이메일 주소를 정의할 수 있습니다.

### DMARC 워크플로 예시

>[!TIP]
>
>DMARC을 _점진적 롤아웃_(으)로 구현하는 것이 좋습니다. 잠재적 영향을 이해하게 되면 DMARC 정책을 `p=none`에서 `p=quarantine`으로, 그 다음 `p=reject`로 확대하고, DMARC 정책을 SPF와 DMKIM에 대한 완화된 정렬로 설정합니다.
...
DMARC는 SPF/DKIM에 실패한 이메일에 대한 보고서를 수신할 수 있는 기능을 제공합니다. ISP 서비스에서 인증 프로세스의 일부로 생성한 두 개의 서로 다른 보고서가 있습니다. 발신자는 DMARC 정책의 RUA/RUF 태그를 통해 이러한 보고서를 받을 수 있습니다.
...
이러한 보고서의 주요 용도는 스푸핑 시도가 있는 이메일에 대한 개요를 받는 것입니다. 매우 기술적인 보고서이며 서드파티 도구를 통해 가장 잘 분석됩니다.
...
Marketo Engage를 통해 전용 IP로 메일을 보내고 브랜드 return-path를 구현하지 않았거나 구현했는지 확실하지 않은 경우 [Adobe 지원 센터](https://experienceleague.adobe.com/home?lang=ko&support-tab=home#support){target="_blank"}를 통해 티켓을 개설하십시오.

>[!BEGINSHADEBOX]

**Journey Optimizer B2B Edition으로 전용 IP 마이그레이션**

전용 IP가 있는 경우 기존 Journey Optimizer 인스턴스와 동일한 영역에서 새 Marketo Engage B2B edition 인스턴스를 만들어야 합니다. 새 인스턴스가 다른 지역에 있는 경우 기존 IP를 공유할 수 없습니다. 지역이 일치하는 경우 [Adobe 지원](https://experienceleague.adobe.com/home?lang=ko&support-tab=home#support){target="_blank"}에서 티켓을 열어 기존 IP 및 바인딩 그룹을 새 인스턴스와 공유하도록 요청하십시오. Marketo Engage 접두사(Munchkin ID)와 새 Journey Optimizer B2B edition 접두사(Munchkin ID)를 입력합니다.

이 요청을 통해 Adobe은 기존 Marketo Engage 인스턴스와 동일한 IP, 바인딩 그룹 및 구성된 반환 경로 도메인을 복제합니다. Marketo Engage 및 Journey Optimizer B2B Edition 인스턴스 간에 IP가 공유되면 동시에 해당 IP를 사용합니다.
...
공유 IP 풀의 고객은 추가 구성이 필요하지 않습니다. 이전과 동일한 IP 풀과 동일한 기본 Return-Path 도메인을 계속 사용합니다.
...
Marketo Engage이 귀하를 대신하여 인터넷 서버에 아웃바운드 연결을 만듭니다. IT 조직과 일부 파트너/공급업체는 허용 목록을 사용하여 서버에 대한 액세스를 제한할 수 있습니다. 이 경우 허용 목록에 추가할 Marketo Engage 아웃바운드 IP 주소 블록을 제공해야 합니다.
