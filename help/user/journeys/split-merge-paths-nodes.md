...
* _사람별 경로 분할_ 노드는 그룹화된 노드입니다. 대상자에 있는 모든 사람이 계정 컨텍스트를 잃지 않고 다음 단계로 이동할 수 있도록 경로가 자동으로 병합됩니다.
* _사람별 분할 경로_ 노드를 중첩할 수 없습니다. 이 그룹화된 노드에 있는 경로에 사람에 대한 분할 경로를 추가할 수 없습니다.
* 분할 경로 노드에는 일치하는 경로가 없는 계정/사람이 여정에서 앞으로 이동하지 않도록 기본 경로를 생략하는 옵션이 포함됩니다.
* _사람별 경로 분할_ 노드는 _계정-사용자 관계_&#x200B;의 사용을 지원하며, 이를 통해 역할 템플릿에 정의된 대로 자신의 역할(계약자 또는 정규직 직원 등)을 기준으로 사용자를 필터링할 수 있습니다.
...
Marketo Engage에서 _스마트 캠페인_&#x200B;은(는) 프로그램 멤버십을 확인하여 잠재 고객이 중복 이메일을 받지 않고 동시에 여러 전자 메일 스트림의 구성원이 아닌지 확인합니다. Journey Optimizer B2B에서 사람들이 분할 경로에 대한 조건으로 Marketo Engage 목록 멤버십을 확인하여 여정 활동의 중복을 제거할 수 있습니다.

이렇게 하려면 **[!UICONTROL 특수 필터]**&#x200B;를 확장하고 **[!UICONTROL 목록의 구성원]** 조건을 필터 공간으로 끌어서 필터 정의를 완료하여 하나 이상의 Marketo Engage 목록에 있는 구성원 자격을 평가합니다.

![Marketo Engage 목록 구성원 자격에 대한 사람 조건별 경로 분할](./assets/node-split-paths-conditions-people-member-of-list.png){width="700" zoomable="yes"}
