...
이 대시보드는 선택한 계정 여정에 대한 포괄적인 개요를 제공하며 완료, 진행 중인 활동 및 시간에 따른 중단을 범주화하고 수량화하는 원 및 선 차트를 사용하여 계정 진행 상황을 자세히 설명합니다. 이는 마케팅 팀이 주요 게재 및 참여 지표를 통해 이메일 및 SMS 채널의 효과를 평가하는 데 도움이 됩니다. 모든 여정에서 집계된 전자 메일 지표의 경우 [전자 메일 성능 보고서](../dashboards/email-performance-dashboard.md)를 참조하십시오.
...
이 섹션에서는 두 가지 참여 지표를 제공합니다.

* **[!UICONTROL 계정별 참여]** - 이 원 차트는 여정의 계정을 _참여_ 및 _미참여_ 범주로 나눕니다. 중앙 그림에는 총 개수가 표시됩니다. 이 시각화는 전반적인 계정 참여를 빠르게 이해할 수 있도록 해줍니다.
...
성능 표는 전자 메일 및 SMS 채널의 효과에 대한 자세한 정보를 제공합니다. 각 표에는 각 통신 접점의 영향을 평가하는 데 도움이 되는 게재 비율 및 클릭스루 비율과 같은 지표가 나와 있습니다. 아래 표에는 이 여정에 대한 이메일 및 SMS 지표만 표시됩니다. 모든 여정에서 동일한 전자 메일 지표의 경우 [전자 메일 성능 보고서](../dashboards/email-performance-dashboard.md)를 사용하십시오.
...
* _[!UICONTROL 자산 이름]_ - 자산의 이름
* _[!UICONTROL 전송됨]_ - 전송된 SMS 메시지 수
* _[!UICONTROL 게재율]_ - 게재된 SMS 메시지 수를 보낸 횟수로 나눈 값입니다.
* _[!UICONTROL 클릭스루 비율]_ - 클릭한 SMS 메시지 수를 게재된 숫자로 나눈 값입니다.
<!--
...
작업 아이콘(**..**)을 사용하여 데이터와 상호 작용합니다. (각 차트 또는 테이블의 오른쪽 상단)
