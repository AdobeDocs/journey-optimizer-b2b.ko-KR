...
AQA( Account Qualification Agent )는 Sales Qualifier 의 핵심입니다. AQA는 AI를 사용하여 계정을 읽고 다음 단계를 위해 준비된 계정을 결정합니다. 조직이 CRM(읽기 전용)을 연결했을 때 리서치, 이메일 드래프팅 및 CRM 정보에 기반한 컨텍스트를 지원합니다.

영업 잠재 고객 및 계정 조사를 위한 ![AI 기반 Account Qualification Agent 대시보드](./assets/acc-qualification-agent.png){width="800" zoomable="yes"}
...
* **초안 전자 메일**
  ...
  잠재 고객 및 계정 통찰력의 연구를 종합하여 이메일 초안을 생성하여 BDR 목표를 기반으로 연관성 있고 개인화된 단일 이메일 콘텐츠를 제작합니다.
...
어떤 계정과 리드가 가장 활발한지 즉시 파악할 수 있으며 가장 높은 의도를 보여 주기 때문에 가장 큰 영향을 미치는 곳에 에너지를 집중할 수 있습니다.

프롬프트를 세분화하여 필요한 결과를 얻음으로써 여정을 반복합니다. 예:

* _수입 통화 또는 보고서와 같은 컨텍스트에서 후속 전자 메일 그림 초안을 작성합니다. 최대 120단어. 제목란: 주요 테마를 통합한 매혹적인 내용 소개: 컨텍스트 소스의 직접 견적과 연결합니다. 신체: 통증 지점에 연결하고 제안을 가치있게 하라. CTA: 더 자세히 살펴보려면 짧은 통화를 제안하십시오._

* _이 전자 메일의 목표는 대화를 시작하고 신뢰도를 높이는 것입니다. 협의적이고 공감 가능한 톤이 있는 120단어로 이메일을 작성하십시오. 지나치게 친숙한 또는 판매 방식을 사용하지 않도록 하고 &quot;괜찮기를 바랍니다&quot;, &quot;체크인만 합니다&quot; 또는 &quot;제발&quot;이라는 문구를 사용하지 마십시오._
...
액세스할 수 있는 모든 잠재 고객 목록을 보려면 왼쪽 탐색에서 **[!UICONTROL 잠재 고객]**&#x200B;을 선택하십시오. 리드 상태 및 마지막 활동과 같은 항목에 대한 빠른 검사를 제공합니다.
...
워크플로 만들기는 5단계 마법사입니다. **목표**, **타깃팅**, **터치포인트 생성**, **설정** 및 **잠재 고객 추가**. 각 단계는 마지막 단계를 기반으로 하며, 정의한 목표는 나중에 결정할 때마다 첫 번째로 형성됩니다.
...
타겟팅 필터는 적격한 잠재 고객을 정의합니다. 나중에 잠재 고객을 추가하면 이러한 필터와 일치하는 사용자만 선택 목록에 나타납니다.
...
이메일 터치포인트를 확장하여 메시지를 읽습니다. 이는 각 잠재 고객의 실제 이메일(톤, 길이, 포커스 및 call to action)을 작성할 때 AI가 따르는 지침입니다.
...
생성된 각 이메일에 대해 **[!UICONTROL 추론]**&#x200B;에서는 콘텐츠와 call to action을 만든 신호, 특성 및 원본을 포함하여 AI가 해당 메시지를 만드는 방법을 설명합니다. 승인하기 전에 이 정보를 검토하여 개인화의 유효성을 검사하십시오.
...
등록을 기준으로 각 터치포인트의 예약된 날짜에 구성된 **표준 시간대**&#x200B;의 워크플로우 **전송 기간** 동안 보낼 전자 메일 큐가 승인되었습니다. 승인하지 않은 잠재 고객은 실행할 때까지 **[!UICONTROL 검토 준비됨]**&#x200B;에 남아 있습니다. 승인 후 워크플로는 정의한 케이던스에 따라 실행됩니다.
...
Sales Qualifier의 _작업_ 영역에서는 BDR(비즈니스 개발 담당자)이 아웃바운드 워크플로 작업을 관리하고 처리할 수 있는 전용 공간을 제공합니다. 작업은 아웃바운드 워크플로 엔진에 의해 자동으로 생성되며 BDR이 각 잠재 고객과 수행해야 하는 특정 작업(전화 통화, LinkedInMails 및 이메일 검토)을 나타냅니다.

작업 관리 경험은 할 일 목록이 아니라 **처리 큐**(으)로 디자인되었습니다. 작업을 열고, 작업을 수행하고, 완료로 표시하고, 페이지를 떠나지 않고 다음 작업으로 이동할 수 있습니다.

전체 작업 페이지를 열려면 왼쪽 탐색 모음에서 **[!UICONTROL 작업]**&#x200B;을 선택하십시오. 작업을 하나씩 처리하기 위한 기본 작업 영역입니다.
...
**전자 메일 검토** — 시스템이 워크플로에 등록된 잠재 고객에 대해 개인화된 전자 메일 생성을 완료하면 만들어집니다. 해당 잠재 고객에 대한 아웃바운드가 시작되기 전에 이메일을 검토하고 승인합니다. 각 잠재 고객은 별도의 이메일 검토 작업을 받습니다. 워크플로에 10개의 잠재 고객을 등록하면 생성이 완료되었을 때 최대 10개의 이메일 검토 작업을 볼 수 있습니다.
...
* **[!UICONTROL 완료 표시]** - 기본 작업입니다. 작업을 실행(호출, InMail 전송 또는 이메일 검토 및 승인)한 후 사용합니다. 완료 시 작업이 **완료됨**(으)로 기록되고 큐가 자동으로 진행됩니다.

* **[!UICONTROL 터치포인트 건너뛰기]** - 작업 패널의 오버플로 메뉴에서 사용할 수 있습니다. 이 특정 단계를 완료할 수 없지만 가망성은 여전히 유효한 대상이므로 워크플로우에 남아 있어야 하는 경우 이 옵션을 사용합니다.

  터치포인트를 건너뛸 때:
   * 전망은 순서의 다음 단계로 나아간다. 향후 작업이 여전히 일정에 따라 생성됩니다.
   * 이유를 선택해야 합니다. *연락처 정보가 잘못됨*, *시기가 잘못됨*, *관련이 없는 콘텐츠* 또는 *기타*(자유 텍스트 필드 포함).
   * 작업 상태가 **건너뜀**(으)로 설정되고 이유 및 타임스탬프와 함께 기록됩니다.
   * 워크플로우의 마지막 단계였던 경우 잠재 고객의 워크플로우 실행이 종료됩니다. 작업이 여전히 건너뜀(제거되지 않음)으로 기록되었습니다.

* **[!UICONTROL 워크플로에서 제거]** - 작업 패널의 오버플로 메뉴에서 사용할 수 있습니다. 잠재 고객이 더 이상 이 워크플로에 있지 않아야 할 때 사용합니다.

  워크플로우에서 잠재 고객을 제거하는 경우:
   * 이 워크플로 내에서 해당 잠재 고객에 대한 보류 중인 모든 작업 및 향후 작업이 취소되었습니다.
   * 잠재 고객의 등록 상태가 **BDR에서 제거됨**(으)로 변경됩니다.
   * 이유를 선택해야 합니다. *왼쪽 회사*, *중복*, *맞지 않음*, *이미 전환됨* 또는 *기타*(텍스트 필드 포함).
   * 확인 대화 상자가 나타납니다. *&quot;[워크플로 이름]에서 [잠재 고객]의 나머지 접점을 모두 취소합니다. 계속하시겠습니까?&quot;*
   * 작업 상태가 **제거됨**(으)로 설정되어 있습니다. 취소된 모든 형제 작업은 **제거됨**(으)로도 표시됩니다.

>[!NOTE]
>
>건너뛰기 및 분석 사유 데이터 피드 제거 - 채널별 건너뛰기 비율, 워크플로우별 제거 비율 및 주요 원인. 이를 통해 워크플로우 품질을 향상시키고 시간에 따른 성능 분석을 알릴 수 있습니다.
...
* **기한 초과** — 기한이 지난 작업이 아직 열려 있습니다. 먼저 이 문제를 해결하십시오.

* **예정** — 이전 워크플로 단계가 이미 완료된 향후 기한의 작업입니다. 이러한 알림은 일찍 표시되므로 적절한 시점(예: 가망 고객과 이미 통화하고 있는 경우)에 미리 계획하거나 더 빨리 행동할 수 있습니다. 계획된 납기가 표시되므로 계획된 타이밍을 알 수 있습니다.
...
예정된 작업은 다음 단계 기한일이 아직 미래인 경우에도 잠재 고객이 워크플로 단계를 완료하는 즉시 생성됩니다. 이를 통해 파이프라인을 조기에 파악할 수 있으므로 기회가 발생할 때 미리 계획하거나 미리 행동할 수 있습니다.
...
>[!IMPORTANT]
>
>Sales Qualifier의 통합에 액세스하려면 `Sales Qualifier Admins` 사용자 그룹의 구성원이 필요합니다.
...
1. 왼쪽 탐색에서 **[!UICONTROL 관리]**&#x200B;를 확장하고 **[!UICONTROL 통합]**&#x200B;을 선택합니다.
   ...
   Salesforce 및 Microsoft Dynamics용 카드가 표시됩니다.
...
* **[!UICONTROL 기본 설정]** - 기본 모임 길이와 백투백 모임 사이에 원하는 시간을 설정합니다.

캘린더 연결을 끊는 경우:

* 활성 예약 링크가 사실상 비활성화되었습니다.
* 예약 페이지에 일시적으로 사용할 수 없는 친숙한 상태가 표시됩니다.
* 다시 연결하면 설정이 유지됩니다.
