...
* _이 전자 메일의 목표는 대화를 시작하고 신뢰도를 높이는 것입니다._ 협의적이고 공감 가능한 톤이 있는 120단어로 이메일을 작성하십시오. 지나치게 친숙한 또는 판매 방식을 피하고 &quot;잘 지내길 바란다&quot;, &quot;체크인만 한다&quot; 또는 &quot;제발&quot;이라는 문구를 사용하지 마십시오.
...
이메일 터치포인트를 확장하여 프롬프트를 읽습니다. 이 지침은 톤, 길이, 포커스 및 call to action 등 각 잠재 고객의 이메일을 작성할 때 AI를 안내합니다.
...
* **잠재 고객별 생성 전에 접점 프롬프트를 완료합니다.**** 대량 생성 후 일반적으로 한 번에 한 개의 잠재 고객이 변경됩니다.
* **이유를 품질 검사로 사용합니다.** 잘못된 신호가 강조되거나 명백한 신호가 누락된 경우 이메일을 편집하거나 터치포인트 프롬프트를 다시 방문하여 케이던스를 다시 생성합니다.
* **편집 도구를 변경 내용에 일치시키십시오.****문구 및 색조에 대한 직접 편집 내용; **[!UICONTROL AI로 생성]**&#x200B;하여 다시 구성하거나 다시 구성하십시오.
* **검토한 내용만 승인하십시오.**** 접점을 확장하고 내용을 읽은 후 등록하기 전에 필요한 위치를 수정하십시오.
