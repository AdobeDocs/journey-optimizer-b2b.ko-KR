...
Journey Optimizer B2B Edition에서 판매 팀이 판매를 담당하는 솔루션을 기반으로 대상 계정 목록에 멤버를 식별하는 구매 그룹을 설정하여 마케팅 효과를 높일 수 있습니다. 마케팅 팀과 함께 구매 그룹을 만들기 전에 주요 구성 요소를 정의해야 합니다. 이들 구성 요소는 비즈니스 목표와 목적을 달성하는 데 중요합니다.
...
* 사용자가 멤버를 구매 그룹에 수동으로 할당한 후 동일한 멤버를 구매 그룹에서 제거하는 트리거된 여정 노드가 뒤따르는 경우, 노드 액션은 해당 멤버를 **제거하지 않으며** 수동 할당을 재정의할 수 없습니다.
* 트리거된 여정 액션 노드가 구매 그룹에 멤버를 추가한 후 구매 그룹에서 동일한 멤버를 제거하는 구매 그룹 유지 관리 작업이 이어지는 경우 유지 관리 작업은 해당 멤버를 **제거하지 않으며** 여정 작업 할당을 재정의할 수 없습니다.
...
완성도 점수는 구매 그룹이 역할에 적합한 멤버를 할당하고 계정 여정에서 사용할 준비가 되었는지 여부를 결정하는 데 사용됩니다. 이 점수는 구매 그룹 내의 역할 수와 최소 하나의 리드가 할당된 역할 수를 기반으로 한 백분율입니다.
...
참여 점수는 구매 그룹 멤버의 활동, 가중치가 적용된 액션, 가중치가 적용된 역할을 기준으로 합니다. 결과 점수는 테넌트/인스턴스 내에서 정규화되어 일관된 비교가 활성화하고 실행 가능한 인사이트를 제공합니다.

최초 참여 점수 계산은 구매 그룹을 생성하는 즉시 시작되며 매일 다시 계산됩니다.
