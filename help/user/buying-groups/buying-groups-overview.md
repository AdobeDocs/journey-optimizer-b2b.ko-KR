...
구매 그룹 참여 점수는 구매 그룹 구성원이 수행하는 활동에 따라 구매 그룹 구성원의 참여를 결정하는 숫자입니다.

* 구매 그룹이 생성되자마자 참여 점수 계산이 시작됩니다.
* 지난 30일 동안 구매 그룹 구성원이 수행한 모든 인바운드 활동은 점수를 계산하는 데 사용됩니다.
* 30일 기간이 있고 활동이 만료되면 점수가 저하될 수 있습니다.
...
| 활동 이름 | 설명 | 참여 유형 | 최대 일별 빈도 수 | 활동 가중치 |
| --- | --- | --- | --- | --- |
| 이벤트 등록 | 캠페인과 연계된 이벤트 등록 | 이벤트 | 20 | 60 |
| 이벤트 참석 | 캠페인 이벤트에 참석합니다. | 이벤트 | 20 | 90 |
| 이메일 열기 | 이메일을 엽니다. | 이메일 | 20 | 30 |
| 이메일 클릭 | 이메일의 링크 클릭 | 이메일 | 20 | 30 |
| 영업 이메일 열기 | 영업 메일 열기 | 이메일 | 20 | 30 |
| Sales Email 클릭 | 판매 이메일의 링크 클릭 | 이메일 | 20 | 30 |
| 즐거운 순간 | 즐거운 시간 보내세요 | 조정됨 | 20 | 60 |
| 푸시 알림 탭 | 푸시 알림 수신 | 모바일 | 20 | 30 |
| 모바일 앱 활동 | 모바일 앱에서 활동을 수행합니다. | 모바일 | 20 | 30 |
| 모바일 앱 세션 | 모바일 앱 세션에서 활성 상태입니다. | 모바일 | 20 | 30 |
| Facebook 리드 광고 양식 작성 | Facebook 페이지에 리드 광고 양식을 작성하여 제출합니다. | Social | 20 | 30 |
| RTP Call to Action 클릭 | 개인화된 클릭 유도 문안 | 웹 | 20 | 60 |
| 인앱 메시지 보기 | 인앱 메시지 보기 | 모바일 | 20 | 30 |
| 인앱 메시지 탭 | 인앱 메시지 탭 | 모바일 | 20 | 30 |
| SMS 구독 | SMS 커뮤니케이션에 구독 | SMS | 20 | 90 |
| 판매 이메일에 회신 | 판매 이메일에 대한 회신 | 이메일 | 20 | 30 |
| 대화 상자 참여 | Dynamic Chat 대화 상자 참여 | 채팅 | 20 | 90 |
| 대화 상자에서 문서와 상호 작용함 | Dynamic Chat 대화 상자에서 문서와 상호 작용합니다 | 채팅 | 20 | 90 |
| 대화 상자에서 예약된 회의 | Dynamic Chat 대화 상자에서 약속 예약 | 채팅 | 20 | 90 |
| 대화 상자 목표에 도달함 | Dynamic Chat 대화 상자에서 목표에 도달 |  | 20 | 90 |
| 웨비나에서 투표에 응답했습니다. | 웨비나 이벤트의 폴에 응답합니다. | 채팅 | 20 | 90 |
| 웨비나에서 클릭한 클릭 유도 문안 | 웨비나 이벤트에서 콜 투 액션 링크를 클릭합니다. | 호출 | 20 | 30 |
| 웨비나의 자산 다운로드 | 웨비나 이벤트에서 파일/자산 다운로드 | 이벤트 | 20 | 60 |
| 웨비나에서 질문하기 | 웨비나 이벤트에서 질문하기 | 이벤트 | 20 | 60 |
| 출석함 이벤트 | 이벤트에 참석함 | 이벤트 | 20 | 60 |
| 대화 상자에서 에이전트와 참여 | Dynamic Chat 대화 상자에서 에이전트와 소통 | 채팅 | 20 | 90 |
| 대화 상자에서 채팅의 링크를 클릭함 | Dynamic Chat 대화 상자에서 링크 클릭 | 채팅 | 20 | 90 |
| 대화형 흐름과 참여 | Dynamic Chat 대화 흐름 참여 | 채팅 | 20 | 90 |
| 대화 흐름의 예약된 회의 | Dynamic Chat 대화 흐름에서 약속 예약 | 채팅 | 20 | 90 |
| 대화형 흐름 목표 도달 | Dynamic Chat 대화 흐름의 목표에 도달 | 채팅 | 20 | 90 |
| 대화형 흐름에서 문서와 상호 작용 | Dynamic Chat 대화 흐름에서 문서와 상호 작용 | 채팅 | 20 | 90 |
| 대화형 흐름에서 에이전트와 참여 | Dynamic Chat 대화 흐름에서 에이전트와 소통 | 채팅 | 20 | 90 |
| 대화 흐름에서 채팅을 클릭함 | Dynamic Chat 대화 흐름의 링크 클릭 | 채팅 | 20 | 90 |
| SMS V2에서 링크 클릭 | SMS 메시지에서 링크 클릭 | SMS | 20 | 90 |

>[!NOTE]
>
>참여 점수 활동은 사용자에 대한 Marketo Engage [활동 로그](https://experienceleague.adobe.com/en/docs/marketo/using/product-docs/core-marketo-concepts/smart-lists-and-static-lists/managing-people-in-smart-lists/locate-the-activity-log-for-a-person){target="_blank"}에 기록됩니다.
