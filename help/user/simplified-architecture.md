...
현재 Marketo Engage 인스턴스의 공유 통신 제한은 API 호출을 통해 설정해야 합니다.

예를 들어 다음과 같은 경우:

* Journey Optimizer B2B edition 인스턴스의 munchkinId는 `JKL-567-MNO`입니다.
* Marketo Engage 인스턴스의 munchkinId는 `ABC-123-DEF`이고 SJ 데이터 센터에 있습니다.

API 요청은 다음과 유사해야 합니다.
