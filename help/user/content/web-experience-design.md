...
>title="찾아보기 모드 사용"
>abstract="이 모드에서는 선택한 웹 채널 구성에 맞게 개인화하려는 정확한 페이지로 이동할 수 있습니다."
...
1. 수정이 완료되면 편집기 위의 왼쪽 화살표를 클릭하여 웹 경험 속성으로 돌아갑니다.

### 요소 수정

표시된 페이지에서 요소를 클릭하여 선택합니다. 파란색 테두리는 선택된 요소를 나타내며 컨텍스트 도구 모음이 수정 옵션과 함께 나타납니다.
...
| **[!UICONTROL 정렬]** | 선택한 요소를 디스플레이 내에서 앞뒤로 이동합니다. |
| **[!UICONTROL 개인화 추가]** | 개인 맞춤화 삽입. |
| **[!UICONTROL 트랙 요소 클릭]** | 요소에 클릭 추적을 추가합니다. |
| **[!UICONTROL 삭제]** | 선택한 요소를 페이지에서 제거합니다. |

선택한 요소의 경우 오른쪽 패널의 속성이 변경되어 사용 가능한 스타일 및 작업을 반영합니다. 패널 위쪽에 있는 작업 아이콘을 클릭하여 선택한 요소를 복제, 클릭-추적, 삭제 또는 숨깁니다.
...
1. 새 텍스트 내용을 입력하거나 텍스트 문자열을 선택하고 바꿀 텍스트를 입력합니다.

1. (선택 사항) 굵게, 기울임꼴, 맞춤 등 [텍스트 서식 옵션](./content-components.md#text)을 사용합니다.
...
* **[!UICONTROL 분할자]** - 이 구성 요소를 사용하여 전자 메일의 레이아웃과 콘텐츠를 구성하는 구분선을 삽입합니다. 오른쪽 패널의 속성에서 선 색상, 스타일 및 높이와 같은 스타일 속성을 조정할 수 있습니다. 자세한 내용은 _콘텐츠 구성 요소_&#x200B;의 [분할기](./content-components.md#divider)를 참조하십시오.
* **[!UICONTROL HTML]** - 이 구성 요소를 사용하여 기존 구조의 HTML 코드를 복사하여 붙여 넣으십시오. 이를 통해 자유 모듈식 HTML 구성 요소를 만들어 일부 외부 콘텐츠를 재사용할 수 있습니다. 자세한 내용은 _콘텐츠 구성 요소_&#x200B;의 [HTML](./content-components.md#html)을(를) 참조하십시오.
* **[!UICONTROL 이미지]** - 이 구성 요소를 사용하여 페이지에 이미지 파일을 삽입합니다. 오른쪽 패널의 속성에서 너비 및 높이와 같은 스타일 속성을 조정할 수 있습니다. 자세한 내용은 _콘텐츠 구성 요소_&#x200B;의 [이미지](./content-components.md#image)를 참조하십시오.
* **[!UICONTROL 제목]** - 이 구성 요소를 사용하여 제목 클래스 텍스트를 삽입합니다. 오른쪽 패널의 속성에서 텍스트 색상, 스타일, 글꼴 및 크기와 같은 스타일 속성을 조정할 수 있습니다. 자세한 내용은 _콘텐츠 구성 요소_&#x200B;의 [텍스트](./content-components.md#text)를 참조하십시오.
* **[!UICONTROL 단락]** - 이 구성 요소를 사용하여 표준 텍스트 요소를 삽입합니다. 오른쪽 패널의 속성에서 텍스트 색상, 스타일, 글꼴 및 크기와 같은 스타일 속성을 조정할 수 있습니다. 자세한 내용은 _콘텐츠 구성 요소_&#x200B;의 [텍스트](./content-components.md#text)를 참조하십시오.
* **[!UICONTROL 링크]** - 지정된 URL에 기본 텍스트 링크를 삽입하려면 이 구성 요소를 사용합니다. 오른쪽 패널의 속성에서 텍스트 색상, 스타일, 정렬 및 크기와 같은 스타일 속성을 조정할 수 있습니다.

왼쪽의 구성 요소 유형을 선택한 다음 추가하려는 위치에 인접한 요소를 마우스로 가리킵니다.

![시각적 편집기 인터페이스 - 새 구성 요소](./assets/web-experience-design-visual-editor-insert-component.png){width="800" zoomable="yes"}
...
삽입할 구성 요소 유형을 선택 해제하려면 페이지 상단에 표시된 파란색 상황에 맞는 배너에서 **[!UICONTROL ESC]**&#x200B;를 클릭하십시오.
...
1. 만들려는 첫 번째 수정 사항을 정의합니다.
   ...
   왼쪽 패널에 기존 수정 사항 목록(있는 경우)이 표시됩니다. 새 수정 사항을 정의하려면 **[!UICONTROL 추가]**&#x200B;를 클릭합니다. 수정 사항이 정의되지 않은 경우 패널은 기본적으로 _[!UICONTROL 수정 사항 추가]_ 옵션으로 설정됩니다.
...
1. **[!UICONTROL 작업 유형]**&#x200B;을(를) 선택하고 필요한 정보/콘텐츠를 지정하십시오.
   ...
   * **[!UICONTROL 콘텐츠 설정]** - _[!UICONTROL CSS 요소 선택기]_ 값으로 식별된 요소의 **[!UICONTROL 콘텐츠]** 필드에 텍스트를 입력합니다.
...
1. 수정 유형으로 **[!UICONTROL 페이지`<head>`]**&#x200B;를 선택하십시오.

1. **[!UICONTROL 콘텐츠]** 상자에 사용자 지정 코드를 추가합니다.

   >[!CAUTION]
   >
   >`<head>` 섹션에는 `<script>` 및 `<style>` 요소만 추가할 수 있습니다. `<div>` 태그 및 기타 요소를 추가하면 나머지 `<head>` 요소가 `<body>` 내에 채워질 수 있습니다.
...
>abstract="이 창을 사용하면 웹 페이지에 정의된 모든 조정 및 추가 사항을 탐색하고 관리할 수 있습니다."

사용자가 만드는 모든 수정 사항은 추적되며 시각적 편집기와 비시각적 편집기 모두의 **[!UICONTROL 수정 사항]** 패널에서 관리할 수 있습니다. 모든 수정 사항을 보려면 왼쪽 도구 모음에서 _[!UICONTROL 수정 사항]_ <!-- ( ![Modifications icon](../assets/do-not-localize/icon-web-exp-modifications.svg) ) -->아이콘을 클릭하십시오.
...
Customer Journey Analytics 및 웹 보고서 작성에 대한 자세한 내용은 [Customer Journey Analytics 설명서](https://experienceleague.adobe.com/en/docs/analytics-platform/using/cja-landing)를 참조하십시오.
