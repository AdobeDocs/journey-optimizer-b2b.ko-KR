...
description: Adobe Journey Optimizer B2B edition이 생성 AI 도구로 생성 또는 편집한 이미지에 Content Credentials을 자동으로 적용하는 방법과 이것이 귀하의 콘텐츠에 어떤 의미가 있는지 알아봅니다.
...
Content Credentials은 이미지 파일로 유지됩니다. 생성 AI로 생성되거나 편집된 이미지를 [!DNL Adobe Journey Optimizer B2B Edition]에 업로드하거나 내보내면 해당 Content Credentials이 유지됩니다.
...
## Content Credentials에 영향을 주는 작업 {#cc-workflows}
...
[!DNL Journey Optimizer B2B Edition]에서 생성 AI 도구를 사용하여 이미지를 생성하거나 편집하면 Content Credentials이 해당 이미지에 자동으로 첨부되며 별도의 작업이 필요하지 않습니다.

### 이미지 생성 {#generate}

**_예제:_** 원하는 시각적 개체를 설명하는 텍스트 프롬프트에서 전자 메일에 대한 배너 이미지를 생성합니다. Content Credentials이 생성된 이미지에 첨부됩니다.

텍스트 프롬프트, 참조 이미지에서 새 이미지를 생성하거나 유사한 이미지를 생성하는 경우 항상 Content Credentials이 첨부됩니다.

### 이미지 자르기 {#crop}

**_예:_**

* 웹 페이지에 맞게 생성된 배너 이미지를 자릅니다. Content Credentials은 자르기 내내 유지됩니다.
* 업로드한 스톡 사진을 이메일 배경으로 사용하고 화면에 맞게 자릅니다. 스톡 사진에 생성 된 AI 정보가 포함되지 않으면 Content Credentials이 생성되지 않습니다.

이미지 파일을 요청된 치수로 자르는 등 조정할 때 소스 이미지에 해당 Content Credentials이 이미 있는 경우에만 이 파일이 유지됩니다. 자르기는 이미지의 픽셀을 다시 만듭니다. 이렇게 하면 일반적으로 해당 Content Credential이 제거되므로, AI 도우미는 자르기 전에 소스 이미지에서 이미지를 읽은 다음 자른 결과에 다시 만들어 다시 연결합니다. 자르기 자체로는 새로운 생성 AI 작업이 추가되지 않으며 기존 AI 작업을 유지합니다.

### 텍스트 오버레이 추가

**_예제:_** 랜딩 페이지의 생성된 배경 이미지에 텍스트 오버레이로 홍보용 헤드라인을 생성합니다. 배경 이미지의 Content Credentials이 유지됩니다.

배경 이미지 위에 생성된 텍스트를 렌더링하면 배경 이미지에 이미 Content Credentials이 있는 경우에만 결과 이미지에 Content Credentials이 첨부됩니다. 오버레이를 렌더링하면 새 이미지가 생성되므로 이미지 편집 도구는 배경에서 Content Credentials을 읽고 결과에 다시 첨부합니다. 오버레이 단계는 새로운 생성 AI 작업을 추가하지 않습니다.

### 이미지 오버레이

**_예:_**

* 생성된 제품 이미지를 생성된 배경과 결합하여 이메일 헤더를 만듭니다. 이 결과는 Content Credentials이 두 가지 생성 AI 소스를 모두 반영한다는 것을 의미합니다.
* 업로드된 두 개의 브랜드 사진을 하나의 콜라주 이미지로 결합합니다. 두 소스 이미지 모두 생성 AI 작업을 전달하지 않으므로 Content Credentials은 생성되지 않습니다.

두 개 이상의 이미지를 함께 합성하고 소스 이미지에 Content Credentials이 있으면 결합된 이미지가 이미지를 유지하고 단일 Content Credentials 메타데이터 요소로 병합됩니다. 합성하면 소스에서 새 이미지가 생성되어 일반적으로 해당 Content Credentials이 제거됩니다. 그러나 이미지 편집 도구는 합성하기 전에 각 요소를 읽은 다음, 생성 AI 작업에 기여한 모든 소스를 나열하는 결합된 단일 Content Credentials 요소를 빌드합니다.
