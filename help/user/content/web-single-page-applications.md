...
개발 팀과 함께 Adobe Web SDK을 사용하여 보기 선언을 구현합니다. 이러한 보기 선언을 사용하면 SPA에서 새 보기로 이동할 때마다 보기 정보가 있는 `sendEvent` 명령을 호출할 수 있습니다.
...
매우 동적인 콘텐츠의 경우, 웹 SDK에는 페이지에 새 요소가 추가되는 시기를 감지하는 돌연변이 관찰자가 포함됩니다. 이러한 관찰자는 요소가 비동기적으로 로드되는 경우에도 수정 사항이 적용되도록 합니다.
...
* **데이터 특성** - 사용자 지정 데이터 특성(`data-testid`, `data-section` 등) 추가 타겟팅할 요소에 매핑할 수도 있습니다.
...
* **요소 가시성** - 요소가 있을 수 있지만 전환 중에 숨겨집니다.
