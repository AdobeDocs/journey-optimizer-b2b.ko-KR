...
[!DNL Journey Optimizer B2B Edition] 디자인 도구를 사용하면 마케터가 **_이메일_** 및 **_랜딩 페이지_**&#x200B;에 대한 콘텐츠를 모두 만들 수 있습니다. WCAG(Web Content Accessibility Guidelines) 2.1, 레벨 AA를 기반으로 이 지시문을 준수하려면 다음 도구를 사용합니다.
...
![글꼴, 크기 및 색상 옵션을 표시하는 텍스트 구성 요소 스타일 탭](assets/accessible-text-styles.png){width="700" zoomable="yes"}

글꼴 및 텍스트의 경우 다음 지침을 따르십시오.
...
콘텐츠에 시각적으로 액세스할 수 있도록 하려면 다음 모범 사례를 따르십시오.
...
[!DNL Journey Optimizer B2B Edition]에서 시각적 디자인 공간 **[!UICONTROL 스타일]** 창의 스타일 매개 변수 및 특성을 사용하여 콘텐츠에 있는 여러 요소의 크기와 간격을 보다 세밀하게 조정할 수 있습니다.

예를 들어 배경을 업데이트하거나 여백, 패딩 및 맞춤을 변경하여 콘텐츠의 시각적 접근성을 향상시킬 수 있습니다.

배경, 여백, 패딩 및 맞춤 설정이 있는 ![스타일 창](assets/accessible-styles.png){width="700" zoomable="yes"}

[!DNL Journey Optimizer B2B Edition] 시각적 디자인 도구를 사용하면 다양한 장치 및 화면 크기에 맞게 디자인을 미리 보고 최적화할 수 있습니다. 언제든지 **[!UICONTROL 실시간 보기로 전환]**&#x200B;하여 다양한 장치 크기에서 콘텐츠가 어떻게 렌더링되는지 확인할 수 있습니다.

![데스크톱, 태블릿 및 모바일 미리 보기 옵션을 표시하는 라이브 보기 전환](assets/accessible-live-view.png){width="700" zoomable="yes"}

>[!CAUTION]
>
>라이브 보기는 다양한 장치 크기에서 렌더링이 어떻게 표시될 수 있는지 비교하도록 설계된 일반 미리 보기입니다. 최종 렌더링은 수신자의 이메일 클라이언트 또는 웹 브라우저에 따라 달라질 수 있다.

## 이미지에 대체 텍스트 사용 {#alt-text}

**[!UICONTROL 이미지]** 구성 요소를 사용하여 이미지에 대체 텍스트를 제공하십시오. 이미지 구성 요소 설정에 대한 자세한 내용은 [_콘텐츠 구성 요소_](content-components.md#image)&#x200B;를 참조하십시오.

대체 텍스트 필드가 강조 표시된 ![이미지 구성 요소 설정 패널](assets/accessible-alt-text.png){width="700" zoomable="yes"}

디지털 제품의 효과적인 대체 텍스트를 보려면 다음 지침을 따르십시오.
...
시각적 디자인 영역에서 관련 [구조](structure-components.md) 및 [콘텐츠](content-components.md) 구성 요소를 사용하여 모든 사용자가 액세스할 수 있는 명확하고 논리적이며 간결한 방법으로 콘텐츠를 구성하십시오. 구조적 상속 모범 사례를 사용하여 **[!UICONTROL 스타일]** 창의 옵션을 설정하십시오.

![구성된 레이아웃의 구조 및 콘텐츠 구성 요소를 보여 주는 전자 메일 디자인 공간](assets/accessible-components.png){width="700" zoomable="yes"}
...
* 일부 사람들이 모를 수 있는 속어, 전문어 또는 지역어는 피하십시오.

콘텐츠 가독성을 평가하려면 Microsoft Word에서 많이 사용되는 [Flesh Reading Ease 테스트](https://support.microsoft.com/en-us/office/get-your-document-s-readability-and-level-statistics-85b4969e-e80a-4777-8dd3-f7fc3c8b3fd2){target="_blank"}를 사용하십시오. 콘텐츠를 0~100의 척도로 얼마나 쉽게 읽을 수 있는지를 계산합니다.

## 콘텐츠 테스트 {#test}

콘텐츠 액세스 가능성을 확인하려면 [!DNL Journey Optimizer B2B Edition]에서 제공하는 테스트 기능을 사용할 수 있습니다. 콘텐츠에 완전히 액세스할 수 있는지 확인하도록 특별히 고안된 것은 아니지만 첫 번째 수준의 확인을 제공할 수 있습니다.
...
![테스트 프로필 미리 보기 옵션이 있는 콘텐츠 시뮬레이션 인터페이스](assets/accessible-simulate.png){width="800" zoomable="yes"}

콘텐츠에 보다 일관된 방식으로 안정적으로 액세스할 수 있는지 확인하려면 다음과 같은 외부 도구를 사용합니다.
...
![어두운 모드를 위한 디자인 설정](assets/email-color-mode-dark-text-settings.png){width="700" zoomable="yes"}

다크 모드 디자인의 경우 투명 PNG 또는 SVG 이미지를 사용하고 적절한 메타 태그와 CSS를 설정합니다. 다크 모드가 지원되지 않는 경우 액세스 가능한 대체 스타일을 제공합니다. 마지막으로 밝은 모드와 어두운 모드 모두에서 모든 콘텐츠 및 UI 요소를 테스트합니다.

전자 메일 디자인 공간에서 _어두운 모드_&#x200B;를 사용하는 방법에 대한 자세한 내용은 전자 메일 콘텐츠에 대한 [_어두운 모드_](./email-dark-mode.md)&#x200B;를 참조하십시오.

## 접근성을 위해 특정 특성 사용 {#attributes}

HTML 속성은 보조 기술이 콘텐츠의 구조, 언어 및 관계를 해석하는 데 도움이 됩니다. 다음 단원에서는 언어 및 텍스트 방향, 레이아웃과 데이터에 사용되는 표, 화면 판독기에서 사용자에게 알리는 링크 텍스트에 대해 설명합니다.
...
화면 판독기는 `lang` 특성을 사용하여 올바른 발음 규칙을 적용합니다. 텍스트 방향을 사용하면 컨텐츠가 왼쪽에서 오른쪽 또는 오른쪽에서 왼쪽 언어로 자연스럽게 흐릅니다. 이러한 속성이 없으면 사용자가 읽기 순서를 혼동하거나 발음을 잘못 이해할 수 있습니다. 따라서 항상 적절한 `lang` 및 `dir` 특성으로 전자 메일 본문을 래핑합니다.
...
HTML 컨텐츠에서 테이블은 종종 레이아웃에 사용됩니다. 기본적으로 화면 판독기는 행, 열 및 구조를 알리는 모든 `<table>`을(를) 데이터 테이블로 취급합니다. 이 구조는 표를 서식에만 사용하는 경우 혼동을 줄 수 있습니다.
...
화면 판독기는 다음과 같이 읽습니다.
&quot;안녕하세요. 이 뉴스레터에 오신 것을 환영합니다.&quot; *(행, 열 또는 테이블에 대한 언급 없음)*
...
화면 판독기는 텍스트를 사용하여 링크를 읽습니다. 링크에 _여기를 클릭_ 또는 _자세한 내용_&#x200B;만 지정되어 있으면 보조 기술 사용자가 대상을 알 수 없습니다. 액세스 가능성을 보장하기 위해 대상이나 작업을 명확하게 나타내는 설명 텍스트가 필요합니다.

디자인 도구를 사용하여 콘텐츠에 대한 링크를 추가하고 레이블을 편집하여 식별 가능(표시) 및 설명(목적에 대해 명확함)하도록 합니다. _여기_ 또는 _자세히_&#x200B;와 같은 모호한 레이블을 사용하지 마십시오.

![URL 필드 및 설명 레이블 옵션을 보여 주는 링크 설정 패널](assets/accessible-link.png){width="700" zoomable="yes"}
