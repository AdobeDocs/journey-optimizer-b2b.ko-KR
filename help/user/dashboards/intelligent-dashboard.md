...
| 유형 | 요구 사항 |
| ---- | ----------- |
| [그룹 단계 구매](#buying-group-stages) | 구매 그룹 단계 **및**&#x200B;을(를) 만들어진 구매 그룹에 추가하도록 설정합니다. |
| [구매 그룹 하이라이트](#buying-group-highlights) | 구매 그룹 단계 **및**&#x200B;을(를) 만들어진 구매 그룹에 추가하도록 설정합니다. |
| [계정 급증](#surging-accounts) | 하나 이상의 게시된 여정 **또는**&#x200B;이(가) 구매 그룹을 만들었습니다. |
| [계정 하이라이트](#account-highlights) | 하나 이상의 게시된 여정 **또는**&#x200B;이(가) 구매 그룹을 만들었습니다. |
| [연락처 범위](#contact-coverage) | 하나 이상의 구매 그룹이 생성되었습니다(단계가 필요하지 않음). |
| [연락처 겹치기](#contact-overlap) | 하나 이상의 구매 그룹이 생성되었습니다(단계가 필요하지 않음). |
| [계정 세부 정보 페이지](../accounts/account-details.md) | 하나 이상의 게시된 여정. |
| [그룹 세부 정보 페이지를 구입하는 중](../buying-groups/buying-group-details.md) | 하나 이상의 구매 그룹이 생성되었습니다(단계가 필요하지 않음). |
...
>title="구매 그룹 단계"
>abstract="이 차트는 구성된 전환 규칙에 따라 다양한 단계에서 구매 그룹 진행의 개요를 제공합니다. 첫 막대는 선택한 시간대의 첫날 특정 단계에 있는 구매 그룹의 수를 선택한 시간대의 마지막 날과 비교하여 나타냅니다."
...
_[!UICONTROL 계정 하이라이트]_ 섹션은 두 행으로 구성되어 조직에 관심 있는 계정에 대한 정보를 표시합니다.
