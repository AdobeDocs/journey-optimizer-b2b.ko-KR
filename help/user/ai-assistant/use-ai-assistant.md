...
AI Assistant 인터페이스가 나타나고 시작하기 위한 정보가 즉시 제공됩니다. _[!UICONTROL 아이디어 시작]_&#x200B;에 제공된 옵션을 사용하여 다음과 같은 질문과 명령에 답변할 수 있습니다.
...
제품 질문을 하려면 패널 하단의 필드에 질문을 입력하고 Enter 키를 누릅니다. 예를 들어 여정에서 구매 그룹을 사용하는 방법을 배워야 하는 경우. 이 경우 _여정에서 구매 그룹을 어떻게 사용합니까?_&#x200B;를 입력하십시오. 질문을 제출하면 AI Assistant가 기술 자료를 쿼리하고 몇 초 안에 답변을 생성합니다.
...
AI Assistant가 작동 중인 샌드박스에 있는지 확인하여 작동 인사이트에 대한 질문에 충분한 응답을 제공합니다.
...
작동 인사이트 질문을 하려면 패널 하단의 필드에 입력한 다음 Enter 키를 누릅니다. 예를 들어 샌드박스의 대상에 대해 알고 싶은 경우. 이 경우 _대상이 몇 명입니까?_&#x200B;를 입력합니다.  AI Assistant는 샌드박스에 있는 대상자 카운트와 대답을 계산한 방법에 대한 설명을 제공합니다. 다음 예제 응답에서 AI Assistant는 UI의 대상에 액세스할 수 있는 링크를 제공하며 대상 수를 식별하는 데 걸린 단계를 간략하게 설명합니다.
...
AI Assistant에서 제공하는 SQL 쿼리를 사용하여 운영 통찰력 질문과 관련된 각 응답을 확인할 수 있습니다. AI Assistant는 작동 인사이트 질문을 통해 쿼리할 때 SQL 쿼리를 제공합니다. 이를 사용하여 답을 계산하는 데 걸린 프로세스를 확인할 수 있습니다. 이 원본 쿼리는 확인 목적으로만 사용되며 쿼리 서비스에서 지원되지 않습니다.

![AI Assistant 쿼리의 결과](./assets/ai-assistant-artifacts-query-source.png){width="550" zoomable="yes"}

## 피드백 제공

응답의 유용성과 품질을 평가하려면 _긍정적 평가_( ![썸네일 위로](../assets/do-not-localize/icon-thumb-up.svg)) 또는 _부정적 평가_( ![썸네일 아래로](../assets/do-not-localize/icon-thumb-down.svg)) 아이콘을 사용하십시오. 경험에 따라 간단한 설문 조사 양식을 작성하고 **[!UICONTROL 제출]**&#x200B;을 클릭합니다. 제공하는 정보는 AI Assistant를 개선하는 데 사용됩니다.
