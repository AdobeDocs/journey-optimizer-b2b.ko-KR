...
| 기능 | 여정 대상에 활성화 | 새로운 _대상에 대한 활성화_ 회사 계정 작업을 사용하여 개인이 아닌 회사에 직접 활성화합니다. (이 릴리스에서는 LinkedIn 회사로 제한됩니다.) [자세히 알아보기](../journeys/action-nodes.md#activate-to-a-linkedin-destination) |
...
| 개선 사항 | 커뮤니케이션 제한 | 이제 시스템에서는 Marketo Engage 및 Journey Optimizer B2B edition의 결합된 통신 제한을 준수합니다. |
| 개선 사항 | 구매 그룹 유지 관리 작업 | 구매 그룹 유지 관리 작업 빈도가 주별에서 일별로 업데이트됩니다. |
| 개선 사항 | 계정 여정 진행률 | 게시된 여정이 _Live_, _새 항목으로 닫힘_, _중단됨_ 또는 _완료됨_ 상태인 경우 여정 맵을 열어 각 여정 노드의 계정 목록을 검토할 수 있습니다. |
