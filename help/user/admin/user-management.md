...
1. Adobe ID 이메일, 사용자 이름, 이름 또는 성을 입력하여 검색합니다.
   ...
   * **[!UICONTROL 관리자 역할]** 열의 값에 `System`이(가) 표시되면 표시된 사용자가 시스템 관리자입니다.
...
>[!TIP]
>
>기존 배포에서 마이그레이션하고 기존 사용자를 Marketo Engage에 추가할 때 사용자 그룹 생성을 건너뛰고 기존 그룹을 열고 Marketo Engage 제품 프로필을 추가하면 됩니다.
...
1. 각 사용자 추가:
   ...
   * 사용자의 이메일 주소, 이름 및 성을 입력합니다.
     ...
     ![Experience Platform - 새 역할에 대한 사용자 프로필 추가](./assets/admin-console-add-users.png){width="600" zoomable="yes"}
...
권한은 제품 프로필에 할당된 권한을 정의할 수 있는 단일 권한입니다. 각 권한은 _여정_ 또는 _구매 그룹_&#x200B;과 같은 기능으로 그룹화됩니다. 이러한 기능은 Journey Optimizer B2B Edition의 함수 또는 개체를 나타냅니다.
...
| B2B 여정 | B2B 여정에 대한 권한을 구성, 관리, 보기 및 게시합니다. 이러한 권한에는 계정 및 사용자 작업, 이벤트 리스너 및 분할 경로가 포함됩니다. | <li>B2B 여정 관리 |
...
| B2B 영업 사용자 | <li>B2B 참여 대시보드 보기 <li>In-CRM Insights 액세스 |

### 역할 권한 편집

기본 제공 또는 사용자 지정 역할의 경우 언제든지 권한을 추가하거나 삭제할 것을 결정할 수 있습니다. 기본 또는 사용자 정의 역할을 수정하는 경우 해당 역할에 할당된 모든 사용자에게 영향을 줍니다.

다음 예에서는 B2B 채널 관리자 역할에 할당된 사용자의 B2B 여정 리소스와 관련된 권한을 추가하려고 합니다. 이 변경 사항으로 해당 역할의 사용자는 계정 여정을 관리할 수도 있습니다.
...
1. **[!UICONTROL 그룹 추가]**&#x200B;를 클릭합니다.
   ...
   ![Experience Platform - 역할에 그룹 추가](./assets/aep-permissions-role-add-groups.png){width="700" zoomable="yes"}
