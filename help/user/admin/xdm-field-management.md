...
고급 전자 메일 개인화에 선택한 필드를 사용하는 방법에 대한 자세한 내용은 [콘텐츠 개인화](../content/personalization.md#custom-datasets)를 참조하십시오. 선택한 필드를 여정 결정에 사용하는 방법(계정별로 경로 분할)에 대한 자세한 내용은 [사용자 지정 데이터 필터링](../journeys/split-merge-paths-nodes.md#custom-data-filtering)을 참조하십시오. <!-- add link to split path by people in M 1.5 GA release -->
