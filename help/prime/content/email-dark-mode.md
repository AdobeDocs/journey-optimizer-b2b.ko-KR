...
이 경우 클라이언트 다크 모드 설정은 [!DNL Journey Optimizer B2B Prime]에서 정의한 사용자 지정 다크 모드 설정보다 우선합니다.

+++

+++사용자 지정 다크 모드를 지원하는 클라이언트

가장 인기 있는 대부분의 전자 메일 클라이언트는 [!DNL Journey Optimizer B2B Prime] 전자 메일 스타일에 사용되는 메서드인 `@media (prefers-color-scheme: dark)` 쿼리를 사용하여 사용자 지정 다크 모드 렌더링을 지원합니다. 이 클라이언트 목록에는 다음이 포함됩니다.
...
이 경우 [!DNL Journey Optimizer B2B Prime]에서 정의한 특정 설정이 렌더링됩니다. 단, 각 이메일 클라이언트에 따라 일부 제한 사항이 적용될 수 있습니다. 예를 들어 일부 클라이언트(예: Apple Mail 16(macOS 13))는 이메일 콘텐츠에 이미지가 있는 경우 다크 모드를 생성하지 않습니다.
...
[!DNL Journey Optimizer B2B Prime] 시각적 디자인 공간에서는 어두운 모드에서 전자 메일 콘텐츠를 스타일링할 수 있는 두 가지 유형의 도구를 제공합니다.
...
1. 이메일 디자인 공간에서 이메일 콘텐츠를 엽니다.
   ...
   ![오른쪽 상단의 조명 모드 선택기를 보여주는 전자 메일 디자인 캔버스](../../user/content/assets/email-color-mode-light-selector.png){width="700" zoomable="yes"}

1. 선택기를 _어두운 모드_( ![어두운 모드 아이콘](../../user/assets/do-not-localize/icon-content-dark-mode.svg) )로 변경합니다.
   ...
   ![다크 모드 선택기와 다크 모드로 표시되는 이메일 콘텐츠를 보여 주는 이메일 디자인 캔버스](../../user/content/assets/email-color-mode-dark-selector.png){width="700" zoomable="yes"}
...
1. 필요한 경우 선택기를 디자인 캔버스의 오른쪽 상단에 있는 _어두운 모드_( ![어두운 모드 아이콘](../../user/assets/do-not-localize/icon-content-dark-mode.svg) )로 이동합니다.

1. 텍스트, 배경 또는 단추와 같은 모든 스타일 색상 속성을 편집합니다.

![색상 및 글꼴 옵션을 보여 주는 어두운 모드 텍스트 스타일 설정 패널](../../user/content/assets/email-color-mode-dark-text-settings.png){width="700" zoomable="yes"}
...
1. 이미지 및 아이콘의 경우 다크 모드에만 특정 에셋을 정의합니다.
   ...
   ![다른 색 조합을 사용하는 아이콘](../../user/assets/do-not-localize/color-contrast-images-diagram.svg){width="80%"}
...
1. 디자인을 변경하는 동안 언제든지 **[!UICONTROL 라이브 보기로 전환]**&#x200B;을 선택하여 콘텐츠가 다양한 장치 크기에서 어떻게 렌더링되는지 확인하십시오.
   ...
   이 보기에서 선택기를 _어두운 모드_( ![어두운 모드 아이콘](../../user/assets/do-not-localize/icon-content-dark-mode.svg) )로 변경하여 다른 장치에서 콘텐츠의 어두운 모드 버전을 미리 봅니다.

   ![다양한 장치 크기에서 어두운 모드로 전자 메일 콘텐츠를 표시하는 실시간 보기 패널](../../user/content/assets/email-color-mode-dark-live-view.png){width="800" zoomable="yes"}
