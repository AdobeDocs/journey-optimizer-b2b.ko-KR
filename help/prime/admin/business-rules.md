...
>title="규칙 유형"
>abstract="채널 규칙 세트에 원하는 규칙 유형을 선택합니다. **빈도 캡핑** 유형을 사용하여 커뮤니케이션 채널에 캡핑 규칙을 적용합니다. 예를 들어 하루에 두 번 이상의 이메일 또는 SMS 커뮤니케이션을 보내지 마십시오. **방방해 금지 시간대(제한된 가용성에서 사용 가능)** 유형을 선택하여 시간 기반 제외를 정의하고 특정 시간대에는 메시지가 전송되지 않도록 합니다."
